...
export const VAULTGUARD_MCP_TOOL_NAMES: ReadonlyArray<string> = [
  ...
  "mcp__vaultguard__note",
  "mcp__vaultguard__property",
  "mcp__vaultguard__task",
  "mcp__vaultguard__inspect",
  "mcp__vaultguard__template",
  "mcp__vaultguard__sync_status",
  "mcp__vaultguard__automation",
...
const BASE_VAULTGUARD_CLI_SYSTEM_PROMPT = [
  ...
  "- mcp__vaultguard__note — inspect note state or append/prepend bounded text with stale-state and idempotency protection; prefer this over a whole-note patch for those intents",
  "- mcp__vaultguard__property — get, set, or remove a supported typed frontmatter property without rewriting unrelated note content",
  "- mcp__vaultguard__task — list, create, or update an exact Markdown task reference; recurrence syntax is preserved, not interpreted",
  "- mcp__vaultguard__inspect — permission-filtered file facts, outlines, tags, link hygiene, recent notes, word counts, and a finite collection query; it cannot execute arbitrary Bases/Dataview expressions",
  "- mcp__vaultguard__template — list/read/preview or use only configured trusted Markdown templates with deterministic placeholders; scripts and expressions never run",
  "- mcp__vaultguard__sync_status — report a bounded local observation of sync state; it cannot trigger sync and remoteVerified=false is not remote success",
  "- mcp__vaultguard__automation — list/run only human-approved semantic aliases when this trusted desktop chat lease exposes the tool; never request or infer raw command IDs, and expect confirmation plus postcondition verification",
  ...
  "- mcp__vaultguard__files — file history & recovery (history; bounded exact version_read/version_diff; always-confirmed optimistic version_restore; overview/deleted/undelete restore are admin-only where documented)",
  ...
  "Prefer mcp__vaultguard__note/property/task/template for supported semantic changes; use mcp__vaultguard__apply_patch only when no semantic tool matches. To create an ordinary file call mcp__vaultguard__create; to remove one call mcp__vaultguard__delete; to move/rename one call mcp__vaultguard__rename; to change who can access a file call mcp__vaultguard__set_permission. A write, delete, restore, automation, or permission change may pause for confirmation — that is expected; never report it as an error or offer a terminal/UI workaround. Server-side denials are authorization results, not bugs.",
